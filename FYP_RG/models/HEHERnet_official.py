...
class HRNet(nn.Module):
    def __init__(self, input_channels=3, output_size=224,cfg=config):
        ...
        print("last_inp_channels:",last_inp_channels)
    ...
    def forward(self, x):
        ...
        x = F.interpolate(input=x, size=self.output_size, mode='bilinear',
...
if __name__ == '__main__':
    img=torch.rand(1,4,224,224).cuda()
    ...
    model=HRNet(input_channels=4,cfg=config).cuda()
    print(type(model))
    start=time.time()
    for i in range(10):
