...
class CameraData:
    ...
    def get_depth(self, img, dpt_mode):
        depth_img = image.Image(img)
        if dpt_mode == "dpt":
          depth_img.img = depth_img.img[:,:,:,1]
    ...
    def get_data(self, rgb=None, depth=None, dpt_mode="ori"):
        depth_img = None
        rgb_img = None
        # Load the depth image
        if self.include_depth:
            depth_img = self.get_depth(img=depth, dpt_mode=dpt_mode)
...
class CameraData_new:
    ...
    def get_depth(self, img, dpt_mode):
        ...
        if dpt_mode == "dpt":
          depth_img.img = depth_img.img[:,:,:,1]
        if self.crop : depth_img.crop(bottom_right=self.bottom_right, top_left=self.top_left)
        #depth_img.normalise()
        depth_img.resize((self.output_size, self.output_size))
        return depth_img.img

    def get_rgb(self, img, normalise=False):
        rgb_img = image.Image(img)
        if self.crop : rgb_img.crop(bottom_right=self.bottom_right, top_left=self.top_left)
        rgb_img.resize((self.output_size, self.output_size))
        return rgb_img.img
    ...
    def get_data(self, rgb=None, depth=None, dpt_mode="ori"):
        ...
        if self.include_depth:
            depth_img = self.get_depth(img=depth, dpt_mode=dpt_mode)
            depth_img = np.reshape(depth_img, (1,self.output_size,self.output_size))

        # Load the RGB image
        if self.include_rgb:
            rgb_img = self.get_rgb(img=rgb)
            rgb_img = np.reshape(rgb_img, (3,self.output_size,self.output_size))
